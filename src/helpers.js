...
export function asteroidVertices (count, rad) {
  ...
  for (let i = 0; i < count; i++) {
    p[i] = {
      x: (-Math.sin((360 / count) * i * Math.PI / 180) + Math.round(Math.random() * 1.2 - 1) * Math.random() / 20) * rad,
      y: (-Math.cos((360 / count) * i * Math.PI / 180) + Math.round(Math.random() * 1.2 - 1) * Math.random() / 20) * rad
    }
  }
  return p
}
